...
#define EXPECT_EQ(a,b) do { uint64_t aa__=(uint64_t)(a),bb__=(uint64_t)(b); if(aa__!=bb__){printf("FAIL:%s:%d got=%llu want=%llu\n",__FILE__,__LINE__,(unsigned long long)aa__,(unsigned long long)bb__);return 1;} } while(0)
...
int main(void) {
    uint8_t backing[512u * 32u], tmp[512u], out[512u];
