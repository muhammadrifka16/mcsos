...
mcsos_blk_status_t mcsos_bcache_init(mcsos_bcache_t *cache, mcsos_bcache_entry_t *entries,
                                     uint32_t entry_count, uint8_t *data_pool, uint32_t block_size) {
...
mcsos_blk_status_t mcsos_bcache_read(mcsos_bcache_t *cache, mcsos_blk_device_t *dev,
                                     uint64_t lba, void *buffer) {
...
mcsos_blk_status_t mcsos_bcache_write(mcsos_bcache_t *cache, mcsos_blk_device_t *dev,
                                      uint64_t lba, const void *buffer) {
    if (cache == 0 || dev == 0 || buffer == 0 || cache->block_size != dev->block_size)
        return MCSOS_BLK_EINVAL;
    mcsos_bcache_entry_t *e = mcsos_bcache_find(cache, dev, lba);
    if (e == 0) {
