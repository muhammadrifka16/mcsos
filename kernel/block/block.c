...
void mcsos_blk_registry_reset(void) {
    for (uint32_t i = 0; i < MCSOS_BLK_MAX_DEVICES; i++) g_blk_devices[i] = 0;
    g_blk_count = 0;
}
...
static mcsos_blk_status_t mcsos_blk_validate_range(mcsos_blk_device_t *dev, uint64_t lba, uint32_t count, const void *buffer) {
    if (dev == 0 || buffer == 0 || count == 0u || dev->ops == 0) return MCSOS_BLK_EINVAL;
    if (lba >= dev->block_count) return MCSOS_BLK_ERANGE;
    if ((uint64_t)count > dev->block_count - lba) return MCSOS_BLK_ERANGE;
    return MCSOS_BLK_OK;
}
